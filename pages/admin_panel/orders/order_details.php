...
<div class="order">
    <div class="order_items">
        <div class="order_info">
            ...
            <h4 class="bigger_h4">Data zamówienia: <?= htmlspecialchars(date('d.m.Y', strtotime($order_details['order_date']))) ?></h4>
        </div>
        <hr/>
        <h4 class="bigger_h4">Dane użytkownika:</h4>
        <div class="user_info">
            <p>Imię: <?= htmlspecialchars($user_details['first_name']) ?></p>
            <p>Nazwisko: <?= htmlspecialchars($user_details['last_name']) ?></p>
            <p>Email: <?= htmlspecialchars($user_details['email']) ?></p>
            <p>Numer telefonu: <?= htmlspecialchars($user_details['phone_number']) ?></p>
        </div>
        <hr/>
        ...
        <div class="payment_method_od">
            <img src="<?= htmlspecialchars($order_details['payment_image_path']) ?>" alt="<?= htmlspecialchars($order_details['payment_method']) ?>"/>
        ...
        <?php foreach ($order_items as $item): ?>
            ...
            <div class="item_price_details_od">
                ...
                <h4>Łączna cena: <?= htmlspecialchars(number_format($item['price'] * $item['quantity'], 2, ',', ' ')) ?> zł</h4>
            </div>
            <hr/>
        <?php endforeach; ?>
    </div>
    <h4 class="bigger_h4 price_summary_od">Łączna cena zamówienia: <?= htmlspecialchars(number_format($order_details['total_price'], 2, ',', ' ')) ?> zł</h4>
</div>